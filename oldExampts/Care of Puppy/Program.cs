...
namespace Care_of_Puppy
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (eat != "Adopted")
            {
                double eatgram = double.Parse(eat);
                totalEat += eatgram;
                eat = Console.ReadLine();
            }
            if (totalEat<=foodGram)
            {
                double different =foodGram - totalEat;
                Console.WriteLine($"Food is enough! Leftovers: {different} grams.");
            }
            else
            {
                double different1 = totalEat - foodGram;
                Console.WriteLine($"Food is not enough. You need {different1} grams more.");
            }
        }
    }
}
